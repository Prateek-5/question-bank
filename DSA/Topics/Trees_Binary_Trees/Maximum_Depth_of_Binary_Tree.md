...
**In plain words:** 1 + max(depth(left), depth(right)).
...
- **What am I being asked to optimize, count, or find?** In this case, we're focused on: 1 + max(depth(left), depth(right)).
...
**The concept:** 1 + max(depth(left), depth(right)).
