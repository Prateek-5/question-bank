...
1. **[Valid_Anagram.md](./Valid_Anagram.md)** — Char count map. **must-do**

### Multi-key hash

2. **[Valid_Sudoku.md](./Valid_Sudoku.md)** — Three sets per category (row, col, box). **must-do**

### Prefix sum + hash (subarray-sum)

3. **[Subarray_Sum_Equals_K.md](./Subarray_Sum_Equals_K.md)** — `count[prefix - k]`. THE canonical pattern. **must-do**
4. **[Maximum_Size_Subarray_Sum_Equals_K.md](./Maximum_Size_Subarray_Sum_Equals_K.md)** — Variant: track first occurrence for max length.
5. **[Largest_Subarray_With_0_Sum.md](./Largest_Subarray_With_0_Sum.md)** — Same idea, target = 0.

### Set-based

6. **[Longest_Consecutive_Sequence.md](./Longest_Consecutive_Sequence.md)** — Build set, start only at run-beginnings (where `n-1` not in set). O(n). **must-do**

### Sliding window — variable size

7. **[Longest_Substring_Without_Repeating_Characters.md](./Longest_Substring_Without_Repeating_Characters.md)** — Window + last-index map. **must-do**
8. **[Minimum_Window_Substring.md](./Minimum_Window_Substring.md)** — Window + need-count + satisfied counter. The hardest window template.

### Hash + clever insight

9. **[Palindrome_Pairs.md](./Palindrome_Pairs.md)** — Map word → index; for each word, check splits where one side is palindrome and other reversed exists.
10. **[Max_Points_on_a_Line.md](./Max_Points_on_a_Line.md)** — Slope hash; tricky equal-points and vertical-line edge cases.
