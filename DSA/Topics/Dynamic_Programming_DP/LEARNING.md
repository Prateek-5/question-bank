...
1. **[Climbing_Stairs.md](./Climbing_Stairs.md)** — `f(n) = f(n-1) + f(n-2)`. THE intro. **must-do**
2. **[Min_Cost_Climbing_Stairs.md](./Min_Cost_Climbing_Stairs.md)** — Variant with costs.
3. **[Maximum_Subarray.md](./Maximum_Subarray.md)** — Kadane: `dp[i] = max(nums[i], dp[i-1] + nums[i])`. **must-do**

### Grid DP

4. **[Unique_Paths.md](./Unique_Paths.md)** — `dp[i][j] = dp[i-1][j] + dp[i][j-1]`. **must-do**
5. **[Unique_Paths_II.md](./Unique_Paths_II.md)** — With obstacles.
6. **[Minimum_Path_Sum.md](./Minimum_Path_Sum.md)** — Cost grid; min-sum path. **must-do**
7. **[Triangle.md](./Triangle.md)** — Bottom-up rows; in-place. **must-do**

### String DP — sequences (LIS, LCS family)

8. **[Longest_Increasing_Subsequence.md](./Longest_Increasing_Subsequence.md)** — `dp[i]` = LIS ending at i; O(n²) or O(n log n) with binary search. **must-do**
9. **[Longest_Arithmetic_Subsequence.md](./Longest_Arithmetic_Subsequence.md)** — `dp[i][diff]` (map per i).
10. **[Russian_Doll_Envelopes.md](./Russian_Doll_Envelopes.md)** — Sort + LIS on heights. **must-do**
11. **[Maximum_Height_by_Stacking_Cuboids.md](./Maximum_Height_by_Stacking_Cuboids.md)** — 3D LIS variant.
12. **[Longest_Common_Subsequence.md](./Longest_Common_Subsequence.md)** — Classic 2D LCS. **must-do**
13. **[Longest_Palindromic_Subsequence.md](./Longest_Palindromic_Subsequence.md)** — LCS of `s` with `reverse(s)`.

### String DP — matching

14. **[Edit_Distance.md](./Edit_Distance.md)** — Insert / delete / replace. THE classic. **must-do**
15. **[Distinct_Subsequences.md](./Distinct_Subsequences.md)** — Count occurrences.
16. **[Decode_Ways.md](./Decode_Ways.md)** — `dp[i] = (single ok ? dp[i-1] : 0) + (double ok ? dp[i-2] : 0)`. **must-do**
17. **[Interleaving_String.md](./Interleaving_String.md)** — 2D DP from two source strings.
18. **[Regular_Expression_Matching.md](./Regular_Expression_Matching.md)** — `.` and `*` cases. Senior bar.
19. **[Frog_Jump.md](./Frog_Jump.md)** — State `(stone, last_jump)`; map of stone → reachable jumps.

### Knapsack

20. **[Partition_Equal_Subset_Sum.md](./Partition_Equal_Subset_Sum.md)** — 0/1 knapsack with target = sum/2. **must-do**
21. **[Ones_and_Zeroes.md](./Ones_and_Zeroes.md)** — 2D knapsack (m zeros, n ones budget).
22. **[Split_Array_with_Same_Average.md](./Split_Array_with_Same_Average.md)** — Subset with average; bitmask DP.
23. **[Minimum_Jumps_to_Reach_Home.md](./Minimum_Jumps_to_Reach_Home.md)** — State-rich BFS/DP hybrid.

### Interval DP

24. **[Matrix_Chain_Multiplication.md](./Matrix_Chain_Multiplication.md)** — `dp[i][j]` = min cost over range. **must-do**
25. **[Unique_Binary_Search_Trees.md](./Unique_Binary_Search_Trees.md)** — Catalan-style interval DP.

### Hybrid / advanced

26. **[Maximal_Rectangle.md](./Maximal_Rectangle.md)** — DP + Largest Rectangle in Histogram (stack). **must-do**
27. **[Dungeon_Game.md](./Dungeon_Game.md)** — **Reverse DP** from bottom-right. Senior bar.

### Digit DP

28. **[Numbers_at_Most_N_Given_Digit_Set.md](./Numbers_at_Most_N_Given_Digit_Set.md)** — Digit-DP template. Senior bar.
